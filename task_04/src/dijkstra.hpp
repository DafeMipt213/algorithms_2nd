...
struct Graph {
  ...
  void addEdge(size_t index_from, size_t index_to, size_t weight, bool reversed = false) {
      nodes[index_from].edges.push_back(Edge(index_to, weight));
      if (reversed)
          nodes[index_to].edges.push_back(Edge(index_from, weight));
  }
...
public:
  ...
  size_t length;
	Dijkstra(Graph& graph, size_t start, size_t finish) {path = findPath(graph, start, finish);}
private:
