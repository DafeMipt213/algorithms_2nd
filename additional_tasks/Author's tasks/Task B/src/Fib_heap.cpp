...
template <class T> FibonacciHeap<T>::FibonacciHeap() { min_ = Empty(); }
...
template <class T> Node<T> *FibonacciHeap<T>::Insert(T key) {
  Node<T> *elem = new Node<T>(key);
  min_ = Merge(min_, elem);
  return min_;
}

template <class T> void FibonacciHeap<T>::Merge(FibonacciHeap &other) {
  min_ = Merge(min_, other.min_);
  other.min_ = Empty();
}

template <class T> bool FibonacciHeap<T>::IsEmpty() { return min_ == nullptr; }

template <class T> void FibonacciHeap<T>::RemoveMinimum() {
  Node<T> *last_min = min_;
  min_ = RemoveMinimum(min_);
  delete last_min;
}

template <class T> void FibonacciHeap<T>::DecreaseKey(Node<T> *n, int key) {
  min_ = DecreaseKey(min_, n, key);
}

template <class T> Node<T> *FibonacciHeap<T>::Find(int key) {
  return Find(min_, key);
}

template <class T> Node<T> *FibonacciHeap<T>::Empty() { return nullptr; }

template <class T> T FibonacciHeap<T>::GetMinimum() { return min_->key; }
...
template <class T> void FibonacciHeap<T>::DeleteAll(Node<T> *heap) {
  if (heap != nullptr) {
    Node<T> *start = heap;
    do {
      Node<T> *n = start;
...
template <class T> void FibonacciHeap<T>::UnMarkMinChild(Node<T> *element) {
  if (element == nullptr) {
    return;
  }
...
template <class T> Node<T> *FibonacciHeap<T>::RemoveMinimum(Node<T> *heap) {
...
template <class T> Node<T> *FibonacciHeap<T>::Cut(Node<T> *heap, Node<T> *n) {
  if (n->right == n) {
...
template <class T>
Node<T> *FibonacciHeap<T>::DecreaseKey(Node<T> *heap, Node<T> *element,
                                       int key) {
  if (element->key < key) {
    return heap;
  }
...
template <class T> Node<T> *FibonacciHeap<T>::Find(Node<T> *heap, int key) {
  auto start = heap;
  if (heap == nullptr) {
    return nullptr;
  }
