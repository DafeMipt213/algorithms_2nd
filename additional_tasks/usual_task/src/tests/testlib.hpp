...
class random_t {
 ...
 public:
  ...
  unsigned long long next(unsigned long long from, unsigned long long to) {
    if (from > to)
      __testlib_fail(
          "random_t::next(unsigned long long from, unsigned long long to): "
  ...
  unsigned long next(unsigned long from, unsigned long to) {
    if (from > to)
      __testlib_fail(
          "random_t::next(unsigned long from, unsigned long to): from can't "
          "not exceed to");
    return next(to - from + 1) + from;
  }
  ...
  typename Iter::value_type any(const Iter &begin, const Iter &end) {
    ...
    if (size <= 0)
      __testlib_fail(
          "random_t::any(const Iter& begin, const Iter& end): range must have "
          "positive length");
  ...
  unsigned long long wnext(unsigned long long n, int type) {
    if (n >= (unsigned long long)(__TESTLIB_LONGLONG_MAX))
      __testlib_fail(
          "random_t::wnext(unsigned long long n, int type): n must be less "
          "LONGLONG_MAX");
  ...
  int wnext(int from, int to, int type) {
    if (from > to)
      __testlib_fail(
          "random_t::wnext(int from, int to, int type): from can't not exceed "
          "to");
  ...
  int wnext(unsigned int from, unsigned int to, int type) {
    if (from > to)
      __testlib_fail(
          "random_t::wnext(unsigned int from, unsigned int to, int type): from "
          "can't not exceed to");
  ...
  long long wnext(long long from, long long to, int type) {
    if (from > to)
      __testlib_fail(
          "random_t::wnext(long long from, long long to, int type): from can't "
          "not exceed to");
    return wnext(to - from + 1, type) + from;
  }
  ...
  unsigned long long wnext(unsigned long long from, unsigned long long to,
    ...
    if (from > to)
      __testlib_fail(
          "random_t::wnext(unsigned long long from, unsigned long long to, int "
  ...
  unsigned long wnext(unsigned long from, unsigned long to, int type) {
    if (from > to)
      __testlib_fail(
          "random_t::wnext(unsigned long from, unsigned long to, int type): "
          "from can't not exceed to");
    return wnext(to - from + 1, type) + from;
  }
  ...
  double wnext(double from, double to, int type) {
    if (from > to)
      __testlib_fail(
          "random_t::wnext(double from, double to, int type): from can't not "
          "exceed to");
  ...
  typename Container::value_type wany(const Container &c, int type) {
    ...
    if (size <= 0)
      __testlib_fail(
          "random_t::wany(const Container& c, int type): c.size() must be "
          "positive");
  ...
  typename Iter::value_type wany(const Iter &begin, const Iter &end, int type) {
    ...
    if (size <= 0)
      __testlib_fail(
          "random_t::any(const Iter& begin, const Iter& end, int type): range "
  ...
  std::vector<T> distinct(int size, T from, T to) {
    ...
    double expected = 0.0;
    for (int i = 1; i <= size; i++) expected += double(n) / double(n - i + 1);

    if (expected < double(n)) {
      std::set<T> vals;
      while (int(vals.size()) < size) vals.insert(T(next(from, to)));
  ...
  std::vector<T> partition(int size, T sum, T min_part) {
    ...
    if (*std::min_element(result.begin(), result.end()) < min_part)
      __testlib_fail(
          "random_t::partition: partition min is expeced to be to less than "
          "the given min_part");

    if (int(result.size()) != size || result.size() != (size_t)size)
      __testlib_fail(
          "random_t::partition: partition size is expected to be equal to the "
          "given size");
...
std::string pattern::next(random_t &rnd) const {
  ...
  if (to == INT_MAX)
    __testlib_fail(
        "pattern::next(random_t& rnd): can't process character '*' for "
        "generation");
  ...
  void writeTestOverviewLog() {
    if (!_testOverviewLogFileName.empty()) {
      ...
      if (fclose(testOverviewLogFile))
        __testlib_fail(
            "Validator::writeTestOverviewLog: can't close test overview log "
            "file (" +
            fileName + ")");
    }
  }
...
static inline double stringToStrictDouble(InStream &in, const char *buffer,
  ...
  if (minAfterPointDigitCount > maxAfterPointDigitCount)
    in.quit(_fail,
            "stringToStrictDouble: minAfterPointDigitCount should be less or "
            "equal to maxAfterPointDigitCount.");
...
static void __testlib_ensuresPreconditions() {
  ...
  if (!__testlib_isNaN(+__testlib_nan()))
    quit(_fail,
         "Function __testlib_isNaN is not working correctly: possible reason "
         "is '-ffast-math'");
  if (!__testlib_isNaN(-__testlib_nan()))
    quit(_fail,
         "Function __testlib_isNaN is not working correctly: possible reason "
...
std::string parseExponentialOptValue(const std::string &s) {
  ...
  if (!num.empty() && num.back() == '.') num = num.substr(0, num.length() - 1);
