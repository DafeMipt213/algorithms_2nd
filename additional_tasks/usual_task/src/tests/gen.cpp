...
bool check_state(const array<int, 10>& cnt) {
  ...
  for (int i = 0; i < 3; ++i) {
    if (!used[i] && (cnt[3 * i + 0] || cnt[3 * i + 1] || cnt[3 * i + 2]))
      return false;
  }
  return true;
}
...
vector<string> gen_brute(const int n, const int a, const int sum_n,
                         const int max_q, const int min_a) {
