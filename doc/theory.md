...
2. Более сильное условие — функция h(v) должна быть `монотонной`. (Эвристическая функция h(v)  называется `монотонной` (или преемственной), если для любой вершины v1  и ее потомка v2 разность h(v1) и h(v2) не превышает фактического веса ребра c(v1,v2) от v1 до v2, а эвристическая оценка целевого состояния равна нулю.)
...
#### Определение потока.
`Потоком` (англ. flow) f в G является действительная функция f:V×V→R, удоволетворяющая условиям:
