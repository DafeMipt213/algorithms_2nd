...
TEST(Jonson, SimpleGraphNoNegativeCycle) {
  ...
  std::vector<std::vector<int>> result = {
      {0, 5, 4, 5, 6, 1}, {1, 0, 5, 6, 7, 2}, {2, 1, 0, 1, 2, 3},
      {7, 6, 5, 0, 1, 2}, {6, 5, 4, 5, 0, 1}, {5, 4, 3, 4, 5, 0}};
  ASSERT_EQ(d, result);
}
...
TEST(Jonson, LittleGraphNoNegativeCycle) {
  std::vector<std::vector<std::pair<int, int>>> graph{
      {{1, 1}}, {{2, 1}}, {{0, 1}}};
  std::vector<std::vector<int>> d = jonson(graph);
  std::vector<std::vector<int>> result = {{0, 1, 2}, {2, 0, 1}, {1, 2, 0}};
  ASSERT_EQ(d, result);
}