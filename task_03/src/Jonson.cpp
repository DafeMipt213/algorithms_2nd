...
bool fordBellman(int s, std::vector<std::vector<std::pair<int, int>>>& graph,
                 std::vector<int>& d) {
  for (int i = 0; i < graph.size(); ++i) d[i] = INF;
  d[s] = 0;
  ...
  for (size_t j = 0; j < graph.size(); ++j) {
    for (size_t i = 0; i < graph[j].size(); ++i)
      if (d[graph[j][i].first] > d[j] + graph[j][i].second) return false;
  }
  return true;
}

void dijkstra(std::vector<std::vector<std::pair<int, int>>>& graph, int s,
              std::vector<int>& d) {
  ...
  for (size_t i = 0; i < graph.size(); ++i) {
    ...
    for (size_t j = 0; j < graph.size(); ++j)
      ...
      if (!colors[j] && (v == -1 || d[j] < d[v])) v = static_cast<int>(j);
    if (d[v] == INF) break;
    colors[v] = 1;
    for (size_t j = 0; j < graph[v].size();
         ++j)  // произведём релаксацию по всем рёбрам, исходящим из v
