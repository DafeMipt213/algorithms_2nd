...
void innerFindBridges(const std::vector<std::vector<int>>& graph, int time, std::vector<int>& tin, std::vector<int>& ret, std::vector<int>& colors, int vertex, int parent, std::set<std::pair<int, int>>& bridges){
  tin[vertex] = ++time;
  ret[vertex] = time;
  colors[vertex] = 1;
  for (int i = 0; i < graph[vertex].size(); ++i)
  {
    ...
    if (colors[graph[vertex][i]] > 0)
      ...
      innerFindBridges(graph, time, tin, ret, colors, graph[vertex][i], vertex, bridges);
...
void innerFindCutPoints(const std::vector<std::vector<int>>& graph, int time, std::vector<int>& tin, std::vector<int>& ret,
                         std::vector<int>& colors, int vertex, int parent, std::set<int>& cut_points){
  int children = 0;
  colors[vertex] = 1;
  tin[vertex] = ++time;
  ret[vertex] = time;
  for(size_t i = 0; i < graph[vertex].size(); ++i){
    ...
    if(colors[graph[vertex][i]] != 0)
      ...
      innerFindCutPoints(graph, time, tin, ret, colors, graph[vertex][i], vertex, cut_points);
      ret[vertex] = std::min(ret[vertex], ret[graph[vertex][i]]);
