...
class Solution {
  std::map<T, int> heights_;
  std::vector<T> eulerian_tour_;
  std::map<T, int> eulerian_tour_position_;

  std::map<T, std::vector<T>> adjacency_list_;
  ...
  void DFS(T vertex, T parent, int current_height, bool is_root = false);
  void BuildLCA();
...
template <typename T>
Solution<T>::Solution(std::map<T, std::vector<T>> &data, T root) {
  int size = data.size();
  for (auto &[vertex, vertexes] : data) {
    eulerian_tour_position_[vertex] = -1;
    heights_[vertex] = 0;
  }
  ...
  DFS(root, root, 0, true);
  BuildLCA();
}

template <typename T>
void Solution<T>::DFS(T vertex, T parent, int current_height, bool is_root) {
  if (adjacency_list_.find(vertex) == adjacency_list_.end()) return;
...
template <typename T>
int Solution<T>::LCAInBlock(int b, int l, int r) {
  return block_rmq_[block_hashes_[b]][l][r] + b * block_size_;
}

template <typename T>
int Solution<T>::LCA(T v, T u) {
  if (adjacency_list_.find(v) == adjacency_list_.end() ||
      adjacency_list_.find(u) == adjacency_list_.end())
    return -1;
