...
std::vector<std::vector<std::pair<size_t, size_t>>> preproc(std::vector<size_t>& data) {
...
std::pair<size_t, size_t> RMQ::findRMQ(std::vector<size_t>& data, size_t l, size_t r) {
		size_t i = fl(r - l + 1);
		size_t id_ans = 0;
        std::vector<std::vector<std::pair<size_t, size_t>>> sparse_table = preproc(data);
