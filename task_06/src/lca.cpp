...
void precompute_lca(int root, AllData &data) {
    ...
    for (int i = 1; i <= m; i++)
        data.log2.push_back(data.log2[i / 2] + 1);

    data.block_size = std::max(1, data.log2[m] / 2);
    data.block_cnt = (m + data.block_size - 1) / data.block_size;

    data.st.assign(data.block_cnt, std::vector<int>(data.log2[data.block_cnt] + 1));
    for (int i = 0, j = 0, b = 0; i < m; i++, j++) {
        if (j == data.block_size)
            j = 0, b++;
    ...
    for (int l = 1; l <= data.log2[data.block_cnt]; l++) {
        for (int i = 0; i < data.block_cnt; i++) {
            ...
            if (ni >= data.block_cnt)
                ...
                data.st[i][l] = MinimumHeights(data.st[i][l-1], data.st[ni][l-1], data);
        }
    }
    ...
    for (int b = 0; b < data.block_cnt; b++) {
        ...
        if (!data.blocks[mask].empty())
            continue;
        data.blocks[mask].assign(data.block_size, std::vector<int>(data.block_size));
        for (int l = 0; l < data.block_size; l++) {
            ...
            for (int r = l + 1; r < data.block_size; r++) {
                ...
                if (b * data.block_size + r < m)
                    data.blocks[mask][l][r] = MinimumHeights(b * data.block_size + data.blocks[mask][l][r], b * data.block_size + r, data) - b * data.block_size;
            }
        }
    }
}
