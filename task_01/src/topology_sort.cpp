...
Graph::Graph(int count_of_verticles) {
  ...
  this->adjacency_matrix =
      std::vector<std::vector<int>>(count_of_verticles, std::vector<int>());
...
void Graph::DFS(int vertex, std::vector<Colors> &visited,
                std::stack<int> &stack) {
...
std::vector<int> Graph::TopologicalSort() {
  ...
  for (int i = 0; i < this->count_of_verticles; i++)
    if (visited[i] == kWhite) DFS(i, visited, stack);
...
void Graph::AddVerticle(int verticle) { this->verticles[verticle] = true; }